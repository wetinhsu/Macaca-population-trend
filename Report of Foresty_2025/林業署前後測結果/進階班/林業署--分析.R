...
dt[1:2]%>% #前測、後測
  ...
  mutate_at(names(.)[31:ncol(.)], function(x){
    ifelse(x != 0, 1,0)
    })%>%
  
  mutate(score_綜合 = (
    apply(select(.,starts_with("綜合1")),1,sum, na.rm = T)+  #對一個1分
...
all_T.test <- 
    ...
    paste0("照片", sprintf("%02.f",1:15)) %>% 
      ...
      mutate(tracect = paste0("照片", sprintf("%02.f",1:15)), .before = 'statistic') %>% 
  
  bind_rows(
    paste0("聲音", sprintf("%02.f",1:15)) %>% 
      lapply(function(x){
      ...
      mutate(tracect = paste0("聲音", sprintf("%02.f",1:15)), .before = 'statistic')
  ) %>% 
  bind_rows(
    
    c("score_照片","score_聲音","score_綜合","總分") %>% 
      lapply(function(x){
      ...
      mutate(tracect = paste0(c("score_照片","score_聲音","score_綜合","總分")) ,
             .before = 'statistic')
