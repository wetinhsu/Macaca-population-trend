...
axis(1, at = 2015:2022, cex.axis=1.5)
axis(2, cex.axis=1.5)
box()
title( "imputed" )
```
...
ggplot(idx[idx$covariate %in% "Overall",], aes(x=time, y=imputed*100)) +
  geom_hline(yintercept=100, colour = "grey", linetype = 2, linewidth = 1)+
...
ggplot(idx[idx$covariate %in% "Overall",], aes(x = time, y= imputed*100 ))+
  ...
  ggsave(filename = paste0("trim_index_MACACA.png"),p, device ="png",
               path =here("./Report of Foresty_20221024"),
...
idx_2022 <- idx[idx$covariate %in% "Overall" & idx$time %in% "2022",]$imputed

round( 100*(idx_2022 -1) ,0)  # idx_2022指標變化值
