...
```{r}
M.Point <- 
  read_excel(here("data/refer/林務局獼猴樣點_20221005.xlsx")) %>% 
...
```{r}
DF<- 
lapply(paste0(here("data/raw/FORESTRYdata/"), "/", 2022), function(x){
...
DF %>%
  filter(is.na(數量)) %>% 
  group_by(`林管處`,`工作站`,`樣區名稱`,`樣區編號`,`旅次`,`備註`) %>% 
  summarise(pp = paste(ordered(as.numeric(樣點編號)), collapse = "、")) %>% 
  mutate(NN = paste0('(樣點',pp,')\n\n',備註)) %>% 
  reshape2::dcast(林管處+工作站+樣區名稱+樣區編號 ~ 旅次, value = NN) %>% 
