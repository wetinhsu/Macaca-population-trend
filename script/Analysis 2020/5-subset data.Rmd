...
```{r}
M.data <- read_excel(here("./data/clean/full_combind_data_V2.xlsx") ) %>% 
...
M.data.1 %>%
  ...
  mutate(TypeName.1 = case_when(
    TypeName %like% "混"         ~ "混淆林",
    TypeName %like% "闊葉樹林型" ~ "闊葉林",
    TypeName %like% "竹林"       ~ "竹林",
    TypeName %like% "針葉樹林型" ~ "針葉林",
  )) %>% 
...
write_xlsx(M.data.2, here("./data/clean/for analysis_V2.xlsx"))
