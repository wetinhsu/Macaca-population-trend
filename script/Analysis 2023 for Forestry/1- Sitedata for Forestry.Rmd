...
list_Point <- list_Point %>% select(-`X_dms`, -`Y_dms`,-`環境特徵描述`)
...
#同一旅次同一樣區內超過7日才完成調查，整個旅次的資料方棄。----
 ...
 DF.2 <- 
  ...
  #為了計算距離，先轉成"數字格式"
  mutate_at(c("TWD97_X", "TWD97_Y", "X_97", "Y_97"), as.numeric) %>% 
  ...
  mutate(point_diff = ifelse(
    ...
    st_distance(
      ...
      st_as_sf(., coords = c("X_97", "Y_97"),
               crs = 3826, remove = F, na.fail = F),#表定位置
      by_element = TRUE) %>% 
      as.numeric()
    )) %>% 
  #再轉回"文字格式"
  mutate_at(c("TWD97_X", "TWD97_Y", "X_97", "Y_97"), as.character) %>% 
...
```{r}
write_xlsx( list('NOTE' = NOTE, 'Data' = M.data), here("data/clean/Forestry/full_combind/full_combind_Forestrydata_2022_V1.xlsx"))
