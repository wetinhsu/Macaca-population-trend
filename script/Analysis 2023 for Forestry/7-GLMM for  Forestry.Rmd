...
```{r}
m1.1 <- glmer(Macaca_sur ~   Year.re + Altitude.1 +   Office  + (1|Site_N), 
            family = binomial, data = df,
            control = glmerControl(optimizer = "bobyqa"))
```
...
Anova(m1.1)  %>% broom::tidy(.) %>% flextable()
```
...
M.data %>% 
  ...
  scale_x_discrete(limits = c( "新竹", "臺中",
                               "南投","宜蘭",
                            "嘉義", "屏東", "花蓮", "臺東"))+
  labs(x = "  ",
       y = "Encounter rate (troop/point)")+
...
```{r eval=FALSE, include=FALSE}
emmip(m1, Office ~ TypeName.1, Is =T)
