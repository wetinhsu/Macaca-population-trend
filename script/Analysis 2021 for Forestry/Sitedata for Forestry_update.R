...
DF<- 
read_excel(path = "./data/clean/Forestry/full_combind/full_combind_Forestrydata_2020_V1.1.xlsx",
             sheet = "Data",
             col_names = T, col_type = "text") %>% 
  select(Office : Date, str_subset(colnames(.), ".ori|校正")) %>% 
...
#同一旅次同一樣區內超過7日才完成調查，整個旅次的資料方棄。----
 ...
 DF.2 <- 
  ...
  #為了計算距離，先轉成"數字格式"
  mutate_at(c("TWD97_X", "TWD97_Y", "經校正_TWD97_X.ori", "TWD97_Y.ori"), as.numeric) %>% 
  ...
  mutate(point_diff = ifelse(
    ...
    st_distance(
      st_as_sf(., coords = c("經校正_TWD97_X.ori", "經校正_TWD97_Y.ori"),
  ...
  #再轉回"文字格式"
  mutate_at(c("TWD97_X", "TWD97_Y", "經校正_TWD97_X.ori", "經校正_TWD97_Y.ori"), as.character)  
...
DF.3 <-  
  ...
  mutate(analysis = case_when(
             ...
             str_detect(備註, "刪除(?!.*斷)") ~ paste0(analysis, ", locate"),
             TRUE ~ analysis)) %>% 
 ...
 M.data <- 
   ...
   relocate(ends_with("校正"), .after = last_col()) %>% 
 ...
 NOTE <- data.frame(
   說明 = c(
     ...
     "4.調查點經2021回傳GPS後，其校正後座標呈現於經校正_TWD97_X.ori與經校正_TWD97_Y.ori兩欄。",
     "5.point.diff是經校正後座標與表定座標的誤差值。",
     "本資料集為林務局獼猴調查資料的合併檔，並已完成清理動作。"
     ))
 
 
  write_xlsx( list('NOTE' = NOTE, 'Data' = M.data), "./data/clean/Forestry/full_combind/full_combind_Forestrydata_2020_V3.xlsx")
