...
ggplot(data = ., aes(x = Year, y = Mean_E/per_Area))+
  geom_point(aes(col = data_source, size = 3))+
  geom_line(aes(col = data_source), linewidth = 1)+
...
TOWN_M_all <- 
  ...
  .[ , 'M_troops' := area*mm/per_Area] %>% 
  .[ , 'q1_troops' := area*q_1/per_Area] %>% 
  .[ , 'q3_troops' := area*q_3/per_Area] %>% 
  .[, 'log_troops':= log(M_troops)]%>% 
  .[ , 'COUNT' := M_troops*25] 
