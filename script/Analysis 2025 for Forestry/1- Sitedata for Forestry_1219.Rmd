...
Office_list <-
  read_excel(
  paste0(here("data/raw/FORESTRYdata/"), "/", 2025,"/林業署獼猴調查樣區列表.xlsx")
  ) %>% 
...
DF<-    
   Office_list %>% 
     ...
     select( names(Office_list),PointID, Point = 樣點代號) %>% 
     right_join(.,DF.o, by = c('Site_N', 'Point'))%>% 

  rows_update(.,Office_list,   by = c("Site_N")) %>% 
  relocate(PointID, .after = "Note")
...
DF %>% 
  ...
  select(ID,analysis, time_diff, day_diff)
...
DF %>% 
 left_join(list_Point,
  ...
  mutate(analysis = case_when(
             point_diff > 51 | is.na(point_diff) ~"locate", 
             # range of gps =50m
             Point %in% "X" ~  "locate",
             # range of gps =50m
             TRUE ~ analysis)) %>% 
  
  select(ID,point_diff,analysis)
...
check_table <- 
DF %>% 
  left_join(dfCheck_data, by = "ID") %>% 
  left_join(DistCheck_data, by = "ID", suffix = c("_1", "_2")) %>%
  mutate(analysis = paste(analysis_1,analysis_2, sep = ",") %>% 
           str_replace_all(., "\\,Y", "") %>% 
           str_replace_all(., "Y,", "") %>% 
           str_trim(.)) %>% 
  select(-analysis_1, -analysis_2) %>% 
  left_join(table_Point[,c('PointID', 'recorded')], by = "PointID") %>%
  select(ID, analysis,time_diff,day_diff,point_diff, 'tagert_recorded'  = recorded) 
...
M.data <-
list_Point %>% 
  select(PointID, TWD97_X, TWD97_Y) %>% 
  left_join(forest3, by = "PointID")  %>%
  left_join(DF,., by = "PointID", suffix = c(".ori", ""))  %>%
   left_join(check_table, by = "ID")%>% 
...
```{r}
write_xlsx( list('NOTE' = NOTE, 'Data' = M.data), here("data/clean/Forestry/full_combind/full_combind_Forestrydata_2025_V1.xlsx"))
