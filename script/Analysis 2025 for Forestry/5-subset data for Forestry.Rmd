...
M.data <- read_excel(here("./data/clean/Forestry/full_combind/full_combind_Forestrydata_2025_V1.xlsx"), sheet = "Data")%>%
       setNames(str_replace_all(names(.),setNames(colname_en,colname_ch))) %>%
...
remove.data <-   #重複記錄的列表
  ...
  filter(!( Year %in% 2025 & Survey %in% 1 & Site_N %in% "MA-F25-10" & Base_point %in% c(3, 9))) %>%
  filter(!( Year %in% 2025 & Survey %in% 2 & Site_N %in% "MA-G28-04" & Base_point %in% c(4))) %>%
  filter(!( Year %in% 2025 & Survey %in% 2 & Site_N %in% "MA-H31-01" & Base_point %in% c(1,5))) %>%
  filter(!( Year %in% 2025 & Survey %in% 1 & Site_N %in% "MA-H34-12" & Base_point %in% c(3))) %>%
    filter(!( Year %in% 2025 & Survey %in% 1 & Site_N %in% "MA-F26-05" & Base_point %in% c(3,5))) %>%
...
write_xlsx( list('NOTE' = NOTE,
                 ...
                 'Data' = M.data.2), 
            paste0(here("./data/clean/Forestry/for analysis/for analysis Forestrydata_2025_V1.xlsx")))
