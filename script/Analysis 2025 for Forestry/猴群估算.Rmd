...
combine_bb %>% 
  ...
  stat_summary(fun = mean, geom = "text", 
               ...
               position = position_nudge(x = 0, y = 0.0065),
               # hjust = 0.3,
               # vjust = -5,
               color = "blue")+
...
town_combine_bb %>% 
  ...
  summarise(Mean_N = mean(N),
              ...
              Se_E = se(E),
            .groups =  'drop') %>% 
  mutate_at(c('Se_N','Se_m','Se_E','Mean_E'), function(x)round(x,3)) %>% 
...
COUNTY__sorted <- town_combine_bb %>% 
 ...
 # .[, median_E := median(Encounter_rate), by =  c('COUNTYNAME')] %>%View
...
town_combine_bb %>% 
  ...
  stat_summary(fun = mean, geom = "text", 
               aes(label = round(Mean_N, 1)),
               position = position_nudge(x = 0, y = 0.07),
                hjust = -0.1,
               # vjust = -9,
               color = "blue")+
