...
#---
   ...
   temp = predict(m2, newdata,re.form=NA,
                  type="response")

   newdata <- 
   cbind(expand.grid(Year.re = 1:5,
   ...
   newdata$pred <- temp
   newdata <- 
     newdata %>% mutate(Year = Year.re + 2014)
 
   newdata %>% 
     group_by(Year) %>% 
     summarise(pred.value = mean(pred)) %>% 
     points(pred.value ~ Year , data = ., type = "o")

  df %>% 
    group_by(Year) %>% 
    summarise(nom.value = mean(Macaca_sur)) %>% 
    plot(nom.value ~ Year , data = ., type = "o", lty=2, ylim = c(0,0.02)) 
   #-----------------------------------------------------------------
   
   

  
  allFit(glmer(Macaca_sur ~ TypeName.1  + factor(Year.re) + Altitude + julian.D +  Region2 + (1|Site_N), 
               family = binomial, data = df))   #嘗試使用一系列優化程序重新擬合glmer模型
  ...
  m3 <- 
  glmer(Macaca_sur ~ TypeName.1  + factor(Year.re) + Altitude.1 + julian.D.1 +  Region2 + (1|Site_N), 
        family = binomial, data = df,
        control = glmerControl(optimizer = "bobyqa"))   
  ...
  d1<- dredge(m3,    trace = T)
  
  
  m4 <- 
    glmer(Macaca_sur ~  Altitude.1 + factor(Year.re) + julian.D.1 +  Region2 + (1|Site_N), 
          family = binomial, data = df,
          control = glmerControl(optimizer = "bobyqa"))   
  ...
  temp = predict(m4, newdata,re.form=NA,
                 type="response")
