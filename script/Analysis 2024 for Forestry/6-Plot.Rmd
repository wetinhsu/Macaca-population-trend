...
M.data %>% 
  ...
  arrange(level2, desc(`2024_prob`) )%>% 
  
  mutate(prob_diff = paste0(`2024_prob`, "(",
                            round( `2024_prob`-`2023_prob`, 1), ")")
         ) %>% 
  
  dplyr::select(-`level2`, -starts_with(c("2020","2021","2022","2023")),
         -`2024_prob`) %>% 
  flextable(.)  %>% 
...
2024年位在海拔50 m以上森林的調查樣點數、臺灣獼猴( *Macaca cyclopis* )的猴群數及相對密度
...
2024年各森林類型內的調查樣點數、臺灣獼猴( *Macaca cyclopis* )的猴群數及相對密度
