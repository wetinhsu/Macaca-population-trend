...
M.data <- read_excel(here("./data/clean/full_combind_data_1525.xlsx") ) %>% 
...
remove.data <- 
  ...
  filter(!( Year %in% 2024 & Survey %in% 1 & Site_N %in% "A25-01" & Base_point %in% 4)) %>% 
  filter(!( Year %in% 2025 & Survey %in% 2 & Site_N %in% "A20-02" & Base_point %in% 3)) %>% 
  # ~手動
...
write_xlsx(M.data.2, here("./合併_計算_20260901/for analysis_1525_20260901.xlsx"))
