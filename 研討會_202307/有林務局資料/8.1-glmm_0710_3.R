...
Ch5_matrix_line_site <- 
  read_xlsx("./研討會_202307/data/clean/BBSsite100mbuffer_0601.xlsx")
...
Df <- 
  ...
  select(transect,year, count, elevation,edge,P_forest,P_farmland,P_Orchard,edge_length) %>% 
  
  mutate(year = year %>% as.numeric(.) %>% as.integer(.))%>% 
  mutate_at(c("elevation","edge","P_forest",
              "P_farmland", "P_Orchard", "edge_length"),scale) %>% 
  data.frame(.) 

write.csv(Df, "./研討會_202307/有林務局資料/data/clean/Df_site_A-C_1522_0710_Orchard.csv", row.names = F)
...
corr <-
  Df %>% 
  na.exclude(.) %>%
  select(year,elevation,edge,P_forest,P_forest, P_Orchard,P_farmland) %>%
...
vif(lm( count ~ year+ elevation+ edge + P_forest+P_Orchard , Df))
...
model2 <- glmmTMB(
  count ~  elevation + edge + P_forest +P_Orchard + (1|year)+(1|transect) ,
  Df,
...
vif(lm( count ~ year+ elevation+ edge + P_farmland +P_Orchard, Df))



model2_2 <- glmmTMB(
  count ~  elevation + edge + P_farmland +P_Orchard + (1|year)+(1|transect) ,
  Df,
  family = binomial(link = "logit")  )
