...
write.csv(Df, "./研討會_202307/有林務局資料/data/clean/Df_site_A-C_1522_0710_Orchard_2.csv", row.names = F)
...
corr <-
  Df %>% 
  na.exclude(.) %>%
  select(year,elevation,edge,P_forest,P_forest,P_farmland, P_Orchard) %>%
...
vif(lm( count ~ year+ elevation+ edge + P_forest + P_farmland+P_Orchard , Df))
...
model2 <- glmmTMB(
  count ~  elevation + edge + P_forest + P_farmland +P_Orchard + (1|year)+(1|transect) ,
  Df,
