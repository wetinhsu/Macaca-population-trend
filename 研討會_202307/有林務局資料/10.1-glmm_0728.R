...
Ch5_matrix_line_site <- 
  read_xlsx("./研討會_202307/有林務局資料/data/clean/BBSsite500mbuffer_0728.xlsx")
...
#write.csv(Df, "./研討會_202307/有林務局資料/data/clean/Df_site_A-C_1522_07281439.csv", row.names = F)

#Df <- read.csv("./研討會_202307/有林務局資料/data/clean/Df_site_A-C_1522_07281439.csv")
...
corr <-
  Df %>% 
  na.exclude(.) %>%
  select(year,elevation,edge,P_forest) %>%
...
vif(lm( count ~ year+ elevation+ edge + P_forest , Df))
...
model2 <- glmmTMB(
  count ~  elevation + edge + P_forest  + (1|year)+(1|transect) ,
  Df,
...
ggplot(Df2, aes(x = P_forest, y = count))+
  geom_point(pch = 21, size = 5.5, col = "#0D2818", fill = "#127475", alpha = .6)+
  geom_smooth(method = "glm",
              method.args = list(family = "binomial"),
              col = "#FB6376",
              size = 2.5,
...
corr_2 <-
  Df %>% 
  na.exclude(.) %>%
  select(year,elevation,edge,P_farmland) %>%
  setNames(., str_remove_all(colnames(.), "P_")) %>% 
  setNames(., str_to_title(colnames(.))) %>% 
  cor(.) %>% 
  round(1)
...
vif(lm( count ~ year+ elevation+ edge + P_farmland , Df))



model2_2 <- glmmTMB(
  count ~  elevation + edge + P_farmland  + (1|year)+(1|transect) ,
  Df,
  family = binomial(link = "logit")  )
