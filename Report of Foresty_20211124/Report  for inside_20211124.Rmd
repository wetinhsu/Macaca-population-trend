...
```
刪除重複記錄前猴群有`r  nrow(M.data[M.data$Year %in% 2021 & M.data$analysis %in% "Y" & M.data$Macaca_dist != "C" &M.data$Macaca_sur.ori %in% 2,]) `群；刪除重複記錄後猴群有`r  sum(M.data[M.data$Year %in% 2021 & M.data$analysis %in% "Y",]$Macaca_sur) `群。  
<br/>  
<br/>  
<br/>    
...
#  filter(Year == 2021) %>% 
  ...
  ggplot(., aes(x = Office, y = E, color = Year)) +
...
#  filter(Year == 2021) %>% 
  ...
  ggplot(., aes(x = TypeName.1, y = E, color = Year)) +
  geom_boxplot(size = 1, width = 0.4, fill= gray(.9))+
...
#   filter(Year == 2021) %>% 
 filter(analysis %in% "Y") %>%
  ...
  ggplot(., aes(x = Altitude_f, y = E, color = Year)) +
  geom_boxplot(size = 1, width = 0.4, fill= gray(.9))+
...
\newpage  
猴群分布的海拔 (max Elevation = `r max(M.data[M.data$Year == 2021 & M.data $Macaca_sur==1 & M.data$analysis == "Y", ]$Altitude, na.rm = T)`)  
```{r echo=FALSE, message=FALSE, warning=FALSE}
tmp <- 
M.data %>% 
  filter(Year == 2021) %>% 
...
Group_than2 %>% 
  ...
  reshape2::dcast(Year+Site_N+Name ~ Survey,
                  value.var = "N") %>% 
  arrange(Site_N) %>% 
  flextable(.) %>% 
   width(., width = c(1,1,2,1,1)) %>% 
