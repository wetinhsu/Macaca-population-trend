...
```{r}
m1 <- glm(Macaca_sur ~  TypeName.1 + Year + Altitude + julian.D +  Region2 + Data  + SiteID, 
...
pre_year_value <-
  predict(m1, newdata = pre_year, type ="link")%>% 
...
```{r}
boxplot(predict_value~ Year , data = pre_year_value)
```
