...
M.data <- 
  ...
  str_subset(paste0(2021:2024, collapse = "|")) %>% #如果只要2020~2022年，就寫2020:2023
  lapply(., read_excel, sheet="Data", col_types = "text") %>% 
  bind_rows() %>% 
  mutate(Office = str_replace_all(Office, "東勢", "臺中")) %>% 
  mutate(Office = str_replace_all(Office, "羅東", "宜蘭")) %>% 
...
ggsave(here("./林務局年報2024/YEAR2.PNG"),
       dpi = 300,
       width = 895,
       height = 879,
       units = "px")
